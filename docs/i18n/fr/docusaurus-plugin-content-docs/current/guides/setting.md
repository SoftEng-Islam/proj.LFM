...
Vous pouvez ouvrir les paramètres sur Files en cliquant sur le bouton `Paramètres` à gauche de Files.
...
Vous pouvez quitter les paramètres de Files en cliquant sur le côté en haut et gauche de Files.
...
Vous pouvez modifier le thème de l'application Files dans l'onglet `Apparence` des Paramètres. Les thèmes disponibles par défaut sont `light`, `light+`, `dark`et `dark+`. Besides, there is a `System Default` theme that will automatically read your system preference, also you can [try custom themes](/docs/Extensions/theme/).
...
You can change Files's font family on the `Appearance` tab of the Settings to one of installed font families installed on your system.

### Taille de la Police

You can change Files's font size on the `Appearance` tab of the Settings. Please note that an ideal font size is between 10px to 30px.

### Transparence de la Fenêtre

Make Files window transparent on the `Appearance` tab of the Settings by combining following options. Please note that an ideal transparency is between 70% to 100%. You can disable transparency by disable all of the transparency options.
...
You can choose the frame style on the `Appearance` tab of the Settings. The available options are `Default` and `System Default`. `Default` will use Files's default style which is the same across platforms. `System Default` will use the system default frame style which is difference according to your platform.
...
:::warning This might causes Files to crash.

This is because Files parses the icon from the exe and if the hexadecimal of the exe file is broken, Files crashes.
...
Any contribution to call it directly from Files is welcome. :::
...
An option to show the info bar on the workspace. ![Files's infobar](/img/docs/infobar.webp)

## Préférences

### App Language

Localize Files. Help us translate Files, [see this discussion](https://github.com/softeng/Files/discussions/30).

### Hide Hidden Files

Hide hidden files on Files, you can find this setting on the `Preference` tab on Files or by its shortcut, `Ctrl + H`.

### Hide System Files

Hide Windows' system files on Files. :::tip Learn what is system file [in this wikipedia](https://en.wikipedia.org/wiki/System_file). Just turn it off if you don't understand what it is. :::

### List and sort directories alongside files

If disabled, Files will prioritize directories above files.
...
Enabling this will make Files to open a file with double click. Otherwise, it will open a file with single click.

### On startup

Option to do on starting Files. Available options are:
