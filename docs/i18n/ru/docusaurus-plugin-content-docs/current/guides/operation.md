...
For current implementation, Files write down what to copy locally (not copying to clipboard).
...
For current implementation, Files write down what to cut locally (not copying to clipboard).
...
Удаленные файлы доступны в пути `Files://Trash` или в корзине вашей системы. :::danger Откройте проблему

Please note that Trash folder cannot be accessed on macos via Files since the [trash crate](https://github.com/Byron/trash-rs) Files relies to does not support it (see [this issue](https://github.com/Byron/trash-rs/issues/8) for more details).

Any contributions to either the trash crate or Files itself for this topic are welcome. :::

### Восстановить файлы

You can restore files by opening the `Files://Trash` and right-clicking it and click the `Restore` option
...
Вы можете мгновенно найти файл набрав начальные символы имени файла/папки, а Files найдет за вас. Some rules of instant search:
...
- При вводе одного и того же символа, Files будет искать следующие совпадения.

## Создать

:::caution Будьте осторожны с именем файла/папки Files воспринимает`/` в имени файла/папки как поддиректорию/подфайл :::
