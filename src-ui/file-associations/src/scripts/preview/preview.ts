...
const createPreviewTable = (icons: IconDefinition[][], size: number) => {
    const table = htmlDoctype + styling + createHTMLTable(createHTMLTableHeadRow(size), createHTMLTableBodyRows(icons));
    return table;
};

const savePreview = async (fileName: string, size: number, icons: IconDefinition[][]) => {
    ...
    createScreenshot(filePath, fileName)
        .then(() => {
            console.log('> Material Icon Theme:', green(`Successfully created ${fileName} preview image!`));
        })
        .catch(() => {
 ...
 */
export const generatePreview = (name: string, icons: IconDefinition[], size: number, trimmableIcons: string[] = []) => {
    savePreview(name, size, getIconDefinitionsMatrix(icons, size, trimmableIcons));
};
 ...
 * @param trimmableIcons List of icons that can possibly be trimmed
 */
const trimIconListToSize = (iconList: IconDefinition[], size: number, trimmableIcons: string[]) => {
    while (iconList.length % size !== 0) {
        iconList.splice(
            iconList.findIndex((i) => i.iconName === trimmableIcons[iconList.length % size]),
            1
        );
    }
};
