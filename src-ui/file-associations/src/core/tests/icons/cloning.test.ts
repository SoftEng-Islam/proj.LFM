...
import { getCloneData, Type, Variant } from '../../generator/clones/utils/cloneData';
import { cloneIcon, getStyle, traverse } from '../../generator/clones/utils/cloning';
import { isValidColor, orderDarkToLight } from '../../generator/clones/utils/color/colors';
import { closerMaterialColorTo, materialPalette as palette } from '../../generator/clones/utils/color/materialPalette';
import { padWithDefaultConfig } from '../../generator/config/defaultConfig';
import { clonesFolder, iconFolderPath, lightColorFileEnding, openedFolder } from '../../generator/constants';
import { getFileConfigHash } from '../../helpers/configHash';
import { merge } from '../../helpers/object';
import { resolvePath } from '../../helpers/resolvePath';
import type { FileIconClone, FolderIconClone, LanguageIconClone } from '../../models/icons/config';
...
describe('cloning: color manipulation', () => {
    ...
    describe('#closerMaterialColorTo(..)', () => {
        ...
        it('should throw an error if the given color is not valid', () => {
            ...
            expect(() => closerMaterialColorTo(color)).toThrowError('The given color "bad-color" is not valid!');
        });
    });
});
        ...
        describe('clone data generation for file icons', () => {
            ...
            it('should create two clone objects if light version exists', () => {
                ...
                const expected = [
                    ...
                    {
                        base: {
                            path: resolvePath(manifest.iconDefinitions!.base2_light!.iconPath),
                            type: Type.File,
                        ...
                        path: resolvePath(`./icons/${subFolder}foo${lightColorFileEnding}${hash}${ext}`),
                        type: Type.File,
                        variant: Variant.Light,
                    },
                ];
        ...
        describe('clone data generation for folder icons', () => {
            it('should create a single clone object if not light version exists or asked', () => {
                ...
                const expected = [
                    {
                        base: {
                            path: resolvePath(manifest.iconDefinitions!['folder-base'].iconPath),
                            type: Type.Folder,
                            variant: Variant.Base,
                        },
                        color: 'green-500',
                    ...
                    {
                        base: {
                            path: resolvePath(manifest.iconDefinitions!['folder-base-open'].iconPath),
                            type: Type.Folder,
                        ...
                        path: resolvePath(`./icons/${subFolder}folder-foo${openedFolder}${hash}${ext}`),
                        type: Type.Folder,
                        variant: Variant.Open,
                    },
                ];
            ...
            it('should create two clone objects if light version exists', () => {
                ...
                const expected = [
                    {
                        base: {
                            path: resolvePath(manifest.iconDefinitions!['folder-base2'].iconPath),
                            type: Type.Folder,
                            variant: Variant.Base,
                        },
                        color: 'green-500',
                    ...
                    {
                        base: {
                            path: resolvePath(manifest.iconDefinitions!['folder-base2-open'].iconPath),
                            type: Type.Folder,
                            variant: Variant.Open,
                        },
                        color: 'green-500',
                    ...
                    {
                        base: {
                            path: resolvePath(manifest.iconDefinitions!['folder-base2_light']!.iconPath),
                            type: Type.Folder,
                            variant: Variant.Light,
                        },
                        color: 'green-500',
                        ...
                        path: resolvePath(`./icons/${subFolder}folder-foo${lightColorFileEnding}${hash}${ext}`),
                        type: Type.Folder,
                        variant: Variant.Light,
                    },
                    {
                        base: {
                            path: resolvePath(manifest.iconDefinitions!['folder-base2-open_light']!.iconPath),
                            type: Type.Folder,
                        ...
                        path: resolvePath(`./icons/${subFolder}folder-foo${openedFolder}${lightColorFileEnding}${hash}${ext}`),
                        type: Type.Folder,
                        variant: Variant.LightOpen,
                    },
                ];
...
const forEachColor = (node: INode, callback: (color: string, loc?: string) => void) => {
    let colorCount = 0;
    ...
    traverse(
        ...
        (node) => {
            ...
            style?.fill && isValidColor(style.fill) && notify(style.fill, 'style:fill');
            style?.stroke && isValidColor(style.stroke) && notify(style.stroke, 'style:stroke');
            node.attributes?.fill && isValidColor(node.attributes.fill) && notify(node.attributes.fill, 'attr:fill');
            node.attributes?.stroke && isValidColor(node.attributes.stroke) && notify(node.attributes.stroke, 'attr:stroke');
