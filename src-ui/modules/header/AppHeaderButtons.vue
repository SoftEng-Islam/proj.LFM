...
div(class="flex items-center h-full gap-x-2 [&>button]:border-none [&>button]:cursor-pointer [&>button]:flex [&>button]:items-center [&>button]:justify-center [&>button]:w-8 [&>button]:h-8 [&>button]:rounded-lg [&>button]:text-base-content [&>button]:text-[16px] [&>button]:transition-all [&>button]:duration-150" v-if="config.appearance.window_controls")
	button(type="button" class="bg-(--color-warning)/20 hover:bg-(--color-warning)" @click="minimizeWindow()" v-if="config.appearance.show_minimize")
		IconMinimize
	button(type="button" class="bg-(--color-success)/20 hover:bg-(--color-success)" @click="toggleMaximize()" v-if="config.appearance.show_maximize")
		IconMaximize
	button(type="button" class="bg-(--color-error)/20 hover:bg-(--color-error)" @click="closeWindow()" v-if="config.appearance.show_close")
		IconClose
</template>
