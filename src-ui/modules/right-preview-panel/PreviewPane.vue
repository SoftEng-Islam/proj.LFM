...
div(class="LFM-preview-pane w-full h-full p-5 flex flex-col gap-y-8 overflow-y-auto bg-(--color-base-200)/80 backdrop-blur-xl border-l border-white/5")
	...
	div(v-else class="flex flex-col gap-y-8")
		...
		section.LFM-preview-section
			...
			div(class="mt-5 p-2 rounded-2xl")
				div(v-if="!isEditingName" @dblclick="startRenaming" class="group flex items-center justify-between cursor-pointer")
					h2(class="text-base font-bold truncate text-(--color-base-content)") {{ selectedItem.name }}
					button(@click="startRenaming" class="group-hover:opacity-100 p-2 transition-opacity hover:text-(--color-primary)")
						IconEdit
				div(v-else class="flex flex-col gap-4")
					input(
						type="text"
						v-model="editedName"
						class="input rounded-2xl bg-(--color-base-100)/60 w-full font-bold border border-dashed border-(--color-primary)/20"
						@keyup.enter="handleRename"
						@keyup.esc="cancelRenaming"
					)
					div(class="flex justify-end gap-2")
						button(class="btn btn-sm rounded-full btn-success text-white px-4" @click="handleRename") Save
						button(class="btn btn-sm rounded-full btn-ghost bg-white/5" @click="resetRename") Reset
						button(class="btn btn-sm rounded-full btn-error text-white px-4" @click="cancelRenaming") Close
