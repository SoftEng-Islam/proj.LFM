...
let
    # Dynamic libraries required by Tauri, its webview, FFmpeg, and MPV at run-time / link-time
  libraries = with pkgs; [
    at-spi2-atk
    atkmm
    cairo
  ...
  packages = with pkgs; [
    ...
    # Metadata and thumbnailing tools
...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    # Avoid WebKitGTK rendering paths that can be unstable on NixOS/Linux.
    ...
    echo "  - ffmpeg / ffprobe / ffmpegthumbnailer (Metadata & thumbnails)"
    echo "  - WebkitGTK 4.1 (UI Rendering)"
    echo "========================================================"
  '';
}
