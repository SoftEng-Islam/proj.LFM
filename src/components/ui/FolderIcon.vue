<script setup lang="ts">
// Size prop must be a number (e.g., 64, not "lg")
// This sets both width and height attributes of the SVG
defineProps<{
	size?: number;
	open?: boolean;
}>();
</script>

<template>
	<svg :width="size ?? 64" :height="size ?? 64" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style="isolation:isolate" version="1.1" viewBox="0 0 48 48" sodipodi:docname="folder-orange.svg" inkscape:version="1.3.2 (091e20ef0f, 2023-11-25)" xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape" xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd" xmlns:svg="http://www.w3.org/2000/svg">
		<defs id="defs3">
			<linearGradient id="a-3" x2="1" y1="0.5" y2="0.5" gradientTransform="matrix(45,0,0,33.7,-59.976328,-7.2813973)" gradientUnits="userSpaceOnUse">
				<stop stop-color="#50c9c3" offset="0" />
				<stop stop-color="#96deda" offset=".983" />
			</linearGradient>
		</defs>

		<linearGradient id="a" x2="1" y1=".5" y2=".5" gradientTransform="matrix(45 0 0 33.7 1.49 12.8)" gradientUnits="userSpaceOnUse">
			<stop stop-color="#50c9c3" offset="0" style="stop-color:#50c9c3;stop-opacity:0;" />
			<stop stop-color="#96deda" offset="0.98299998" style="stop-color:#ffffff;stop-opacity:0.40000001;" />
		</linearGradient>
		<path id="path4" d="M 13.889648 11.469727 L 6.4995117 11.47998 C 5.1595131 11.47998 3.9395499 12.008864 3.0395508 12.859863 C 2.3135515 13.500863 1.5 14.819435 1.5 16.419434 L 1.5 41.519531 C 1.5 44.249529 3.7395145 46.460449 6.4995117 46.460449 L 41.500488 46.460449 C 44.260486 46.460449 46.5 44.249529 46.5 41.519531 L 46.5 21.319336 C 46.5 18.589339 44.260486 16.379883 41.500488 16.379883 L 26.271973 16.388672 C 22.849568 16.260671 20.22115 14.524367 17.519531 12.619629 C 16.620532 11.98763 14.999647 11.469727 13.889648 11.469727 z " style="fill:#f89406;fill-opacity:1" />
		<path d="m 6.5,11.48 c -1.34,0 -2.56,0.529 -3.46,1.38 -0.6998917,2.126911 0.202267,3.266682 2.23,3.54 h 21.3 C 23,16.3658 20.3,14.58 17.52,12.62 16.621,11.988 15,11.47 13.89,11.47 Z M 2.37,13.65 C 2.36167,13.6621 2.3533,13.674 2.3451,13.6862 2.35325,13.674 2.3618,13.6621 2.37,13.65 Z m -0.202,0.324 c -0.0434,0.0748 -0.085,0.15 -0.124,0.227 0.0396,-0.0785 0.0822,-0.153 0.124,-0.227 z M 1.975,14.345 C 1.9207,14.46 1.871,14.577 1.826,14.697 1.8718,14.576 1.9225,14.459 1.975,14.345 Z M 1.783,14.814 C 1.7384,14.941 1.6999,15.072 1.666,15.203 1.7008,15.07 1.7391,14.94 1.783,14.814 Z m -0.148,0.504 c -0.03,0.129 -0.0522,0.261 -0.0718,0.394 0.02,-0.133 0.0418,-0.266 0.0718,-0.394 z m -0.0981,0.563 c -0.02,0.179 -0.0322,0.361 -0.0322,0.546 -3.73e-4,-0.186 0.012,-0.368 0.0322,-0.546 z" fill="#37a29c" sodipodi:nodetypes="ccccccccccccccccccccccccc" style="fill:#000000;fill-opacity:0.19999999" />
		<path d="m5.27 16.4c-2-0.266-2.92-1.39-2.23-3.54-0.726 0.641-1.54 1.96-1.54 3.56v25.1c0 2.73 2.24 4.94 5 4.94h35c2.76 0 5-2.21 5-4.94v-20.2c0-2.73-2.24-4.94-5-4.94z" fill="url(#a)" />
	</svg>
</template>
