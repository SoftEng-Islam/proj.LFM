...
const iconLoaders = import.meta.glob('/src/file-associations/icons/*.svg', {
	import: 'default',
	query: '?url',
...
export function resolveFileIconName({ fileName, filePath }: ResolveFileIconInput): string {
	...
	const fileNameMatch = fileNameAssociations.find(
		(association) =>
			baseName === association.fileName || normalizedPath.endsWith(`/${association.fileName}`)
	);
	if (fileNameMatch) return fileNameMatch.iconName;

	const extensionMatch = extensionAssociations.find(
		(association) =>
			baseName === association.extension || baseName.endsWith(`.${association.extension}`)
	);
