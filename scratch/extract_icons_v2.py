...
def get_fill_color(content):
    ...
    if match:
        return match.group(1)
    # Try another pattern
    match = re.search(r'path[^>]*id="path4"[^>]*style="[^"]*fill:([^;"]+)', content)
    if match:
        return match.group(1)
    return None
...
def extract_tag_data(content, tag_name):
    ...
    for m in matches:
        markup = m.group(0)
        # Check if it's a folder part
        if 'id="path4"' in markup or 'id="path2"' in markup or 'id="path3"' in markup:
            continue
        # Also check for variations (path2-6, path4-3 etc)
...
with open('scratch/folder_icons_data_v2.json', 'w') as f:
